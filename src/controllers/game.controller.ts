...
export const leaderboard = async (req:Request, res:Response):Promise<any> => {
    try {
        const conn = await getConnection();
        const playerStats = await conn?.request()
        .input('num', sql.Int, 5)
...
export const match = async (req:Request,res:Response):Promise<any> => {

    const {user_id, score_rapidez, score_seguridad, score_protocolo, 
        score_razonamiento, questions_answered, time_left_seconds} = req.body;
